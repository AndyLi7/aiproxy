...
func TestCompiledSeedreamContract(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		body := mustMarshalJSON(t,
			map[string]any{
				"model":  "bytedance/seedream-5.0-lite/edit",
				"prompt": "A cup",
				"image":  []string{"https://example.com/a.png"},
				"size":   tc.size,
			},
		)
...
func TestCompiledSeedreamTransportParameters(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		normalized, validationErr := ValidateImage(
			contract,
...
func TestValidateImageBoundariesAndDefaults(t *testing.T) {
	for _, size := range []string{"1500x1500", "0x0", "8192x8192", "32000x128"} {
		_, err := ValidateImage(
			...
			"vendor/image/edit",
			[]byte(
				`{"model":"vendor/image/edit","prompt":"cup","image":["https://example.com/a.png"],"size":"`+size+`"}`,
			),
		)
	...
	body, err := ValidateImage(
		...
		"vendor/image/edit",
		[]byte(
			`{"model":"vendor/image/edit","prompt":"cup","image":["https://example.com/a.png"]}`,
		),
	)
	require.Nil(t, err)
...
func TestValidateImageCombinedCountAndRegistryChange(t *testing.T) {
	...
	body := mustMarshalJSON(t,
		map[string]any{
			...
			"sequential_image_generation_options": map[string]any{"max_images": 2},
		},
	)
	...
	schema, ok := changed["input_schema"].(map[string]any)
	require.True(t, ok)
	constraints, ok := schema["x-image-constraints"].(map[string]any)
	require.True(t, ok)
	combined, ok := constraints["combinedImages"].(map[string]any)
	require.True(t, ok)

	combined["maximum"] = 16
	contract := mustMarshalJSON(t, changed)

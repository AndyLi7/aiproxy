...
func TestCompiledMediaProviderContract(t *testing.T) {
	...
	mapped, err := MapBoundProviderInput(raw, b, "fal-image", s.Endpoint, "async", []byte(`{"model":"test/media/edit","prompt":"test","n":2,"max_images":3,"image_urls":["https://example.com/a.png","https://example.com/b.png"]}`))
	if err != nil {
		t.Fatal(err)
	}
	evidence, err := ResolveImageMetering(raw, b, mapped, 2, 1024, true)
	if err != nil || evidence.InputCount != 2 || evidence.MaximumOutputs != 6 || !evidence.Explicit {

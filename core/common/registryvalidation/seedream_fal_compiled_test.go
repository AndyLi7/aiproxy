...
func TestCompiledFalSeedreamContracts(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			input := map[string]any{"model": "public/seedream", "prompt": "A presentation slide", "n": 2, "image_size": "landscape_16_9"}
			if tc.batch {
				input["max_images"] = 3
			} else {
			...
			if tc.edit {
				input["image_urls"] = []string{"https://example.com/a.png", "https://example.com/b.png"}
			}
			...
			if native["num_images"] != float64(2) || native["sync_mode"] != false || native["enable_safety_checker"] != true {
				t.Fatalf("bad native request: %s", mapped)
			}
			metering, err := ResolveImageMetering(raw, b, mapped, 2, 1024, true)
			if err != nil || metering.InputCount != tc.inputs || metering.MaximumOutputs != tc.outputs {
				t.Fatalf("%+v %v", metering, err)
			}
			delete(input, "image_size")
			delete(input, "output_format")
			withoutDefaults, _ := json.Marshal(input)
			if _, err := MapBoundProviderInput(raw, b, "fal-image", s.Endpoint, "async", withoutDefaults); err == nil {
			...
			if tc.batch {
				...
				if _, err := MapBoundProviderInput(raw, b, "fal-image", s.Endpoint, "async", withoutMultiplier); err == nil {
			...
			if tc.edit {
				...
				if _, err := MapBoundProviderInput(raw, b, "fal-image", s.Endpoint, "async", bad); err == nil {
			...
			if tc.batch && tc.edit {
				...
				input["image_urls"] = []string{
					"https://example.com/1.png", "https://example.com/2.png", "https://example.com/3.png", "https://example.com/4.png", "https://example.com/5.png",
					"https://example.com/6.png", "https://example.com/7.png", "https://example.com/8.png", "https://example.com/9.png", "https://example.com/10.png",
				}
				input["n"] = 1
				input["max_images"] = 5
				boundary, _ := json.Marshal(input)
				if _, err := ValidateImage(raw, "bytedance/seedream-4.5-v2/edit", boundary); err != nil {
				...
				mappedBad, err := MapBoundProviderInput(raw, b, "fal-image", s.Endpoint, "async", bad)
				if err != nil {
					t.Fatal(err)
				}

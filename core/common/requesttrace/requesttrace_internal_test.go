...
func TestRecorderEmitsValidLifecycle(t *testing.T) {
	...
	if saved[1].Revision != 2 || saved[1].SpanID != saved[0].SpanID ||
		saved[1].StartedAt != saved[0].StartedAt {
...
func TestRecorderEntropyFailureReturnsSafeDisabledHandle(t *testing.T) {
	...
	if r.TraceID() != "" || emitted != 0 {
		t.Fatalf(
			"entropy failure leaked fallback identity or event: trace=%q emitted=%d",
			r.TraceID(),
			emitted,
		)
	}
...
func TestRecorderSpanEntropyFailureDisablesRecorder(t *testing.T) {
	...
	if first == nil || first.Finish(StatusSuccess) || second == nil ||
		second.Finish(StatusSuccess) {
...
func TestValidateRejectsUnsafeContractValues(t *testing.T) {
	...
	tests := []struct {
		...
		{"negative duration", func(s *Span) { s.DurationMS = new(-0.1) }},
		{"NaN duration", func(s *Span) { s.DurationMS = &nan }},
		{"infinite duration", func(s *Span) { s.DurationMS = &inf }},
		{
			"long public model",
			func(s *Span) { s.Attributes.PublicModelID = strings.Repeat("m", 129) },
		},
		{
			"attribute control character",
			func(s *Span) { s.Attributes.UpstreamModelID = "model\tsecret" },
		},
...
func TestJSONRejectsUnknownFields(t *testing.T) {
	...
	tests := []string{
		...
		strings.Replace(
			string(validJSON),
			`"attributes":{`,
			`"attributes":{"response_body":"secret",`,
			1,
		),
	}
...
func validCompletedSpan() Span {
	started := time.Date(2026, time.September, 8, 1, 2, 3, 0, time.UTC)

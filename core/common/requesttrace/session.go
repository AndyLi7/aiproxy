...
func (s *Session) BindVerifiedContext(groupID string, verified VerifiedContext) bool {
	if s == nil || verified.claims.GroupID != groupID || groupID == "" ||
		verified.claims.RequestID != s.recorder.requestID ||
		!validRandomID(verified.TraceID()) ||
		!validRandomID(verified.ParentSpanID()) {
		return false
	}
...
func (s *Session) BindStoredTask(groupID, traceID, parentID string) bool {
	if s == nil || s.recorder == nil ||
		validateRequiredString("group", groupID, maxGroupIDBytes) != nil {
		return false
	}

	return s.BindVerifiedContext(
		groupID,
		VerifiedContext{
			claims: TrustedClaims{
				GroupID:      groupID,
				RequestID:    s.recorder.requestID,
				TraceID:      traceID,

...
func TestSessionBuffersUntilOwnerIsBound(t *testing.T) {
	...
	s := NewSession(
		"same-client-request",
		func(v Span) bool { saved = append(saved, v); return true },
	)
...
func TestRecorderAttributesAreDeepCopied(t *testing.T) {
	...
	for _, span := range saved {
		if *span.Attributes.ChannelID != 7 || *span.Attributes.Attempt != 2 ||
			*span.Attributes.HTTPStatus != 202 ||
			*span.Attributes.Width != 512 ||
			*span.Attributes.Height != 768 ||
			*span.Attributes.Seconds != 1.5 ||
			!*span.Attributes.GenerateAudio {
...
func TestRecorderRejectedStartCanStillEmitCompletion(t *testing.T) {
	...
	if !h.Finish(StatusSuccess) || len(saved) != 1 || saved[0].Revision != 2 ||
		!saved[0].Truncated {
...
func TestSessionConcurrentLifecycleAndAttempts(t *testing.T) {
	...
	s := NewSession(
		"request",
		func(v Span) bool { emitMu.Lock(); saved = append(saved, v); emitMu.Unlock(); return true },
	)

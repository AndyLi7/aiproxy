...
const (
	trustedContextDomain   = "aiproxy-request-trace-v1."
	maxTrustedContextBytes = 2048
)
...
type TraceNonceConsumer interface {
	ClaimTraceNonce(ctx context.Context, nonce string, expiresAt time.Time) (bool, error)
}
...
func VerifyTrustedContext(
	ctx context.Context,
	wire string,
	keys map[string][]byte,
	actual TrustedRequest,
	now time.Time,
	nonces TraceNonceConsumer,
) (VerifiedContext, error) {
	if ctx == nil || ctx.Err() != nil || nonces == nil || len(wire) == 0 ||
		len(wire) > maxTrustedContextBytes {
	...
	if c.GroupID != actual.GroupID || c.RequestID != actual.RequestID ||
		c.Method != actual.Method ||
		c.Path != actual.Path {
		return VerifiedContext{}, errTrustedContext
	}
	...
	accepted, err := nonces.ClaimTraceNonce(
		claimCtx,
		hex.EncodeToString(digest[:]),
		time.Unix(c.IssuedAt+121, 0).UTC(),
	)
...
func validTrustedClaims(c TrustedClaims) bool {
	if c.Version != 1 || c.IssuedAt <= 0 || !validRandomID(c.TraceID) ||
		!validRandomID(c.ParentSpanID) ||
		!validRandomID(c.Nonce) {
		return false
	}
	...
	if validateRequiredString("key", c.KeyID, 64) != nil ||
		validateRequiredString("group", c.GroupID, maxGroupIDBytes) != nil ||
		validateRequiredString("request", c.RequestID, maxRequestIDBytes) != nil {
		return false
	}

	if c.Method != http.MethodPost && c.Method != http.MethodGet && c.Method != http.MethodDelete {
		return false
	}

	return strings.HasPrefix(c.Path, "/v1/") && !strings.ContainsAny(c.Path, "?#") &&
		validateRequiredString("path", c.Path, 512) == nil
}

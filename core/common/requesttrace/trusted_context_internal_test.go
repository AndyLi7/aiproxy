...
func (s *contextNonceSink) ClaimTraceNonce(
	_ context.Context,
	digest string,
	expires time.Time,
) (bool, error) {
...
func TestTrustedContextRoundTripBindsAuthenticatedRequest(t *testing.T) {
	...
	verified, err := VerifyTrustedContext(
		context.Background(),
		wire,
		map[string][]byte{"platform-1": key},
		TrustedRequest{
			GroupID:   "account-a",
			RequestID: "request-a",
			Method:    "POST",
...
func TestTrustedContextStorageUnavailableReturnsNoContext(t *testing.T) {
	...
	verified, err := VerifyTrustedContext(
		...
		now,
		&contextNonceSink{err: errors.New("storage unavailable")},
	)
	require.Error(t, err)

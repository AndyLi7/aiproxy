...
func TestWriterOptionsDefaultAndCapBounds(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:        WriterOptions{QueueSize: -1, WriteTimeout: -1, MaxAttempts: -1},
			queueSize: 4096,
			timeout:   time.Second,
			attempts:  2,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cap(w.queue) != tt.queueSize || w.writeTimeout != tt.timeout ||
				w.maxAttempts != tt.attempts {
				t.Fatalf(
					"normalized options: queue=%d timeout=%s attempts=%d",
					cap(w.queue),
					w.writeTimeout,
					w.maxAttempts,
				)
...
func TestWriterConcurrentSubmitAndClose(t *testing.T) {
	w := NewWriter(
		sinkFunc(func(context.Context, Span) error { return nil }),
		WriterOptions{QueueSize: 256, MaxAttempts: 1},
	)

...
func ImageTasks() []gin.HandlerFunc {
	return []gin.HandlerFunc{replayImageTask, middleware.NewDistribute(mode.ImagesGenerations), submitImageTask}
}
...
func GetImageTask(c *gin.Context) {
	...
	task, err := model.GetImageTask(c.Param("id"), middleware.GetGroup(c).ID, middleware.GetToken(c).ID)
...
func submitImageTask(c *gin.Context) {
	...
	if json.Unmarshal(encoded, &wrapper) != nil || wrapper.Contract.Execution.Mode != "async" || wrapper.Contract.Execution.Output != "image" {
	...
	mt := NewMetaByContext(c, selected.channel, mode.ImagesGenerations)
	...
	requiredBalance := math.Max(consume.CalculateAmountWithOptions(
		...
		price,
		model.PriceSelectionOptions{DisableResolutionFuzzyMatch: mc.DisableResolutionFuzzyMatch, RequestAt: requestAt},
	...
	info := &model.AsyncUsageInfo{RequestID: id, RequestAt: requestAt, UsageContext: requestUsage.Context, Mode: int(mode.ImagesGenerations), Model: mt.OriginModel, Capability: middleware.GetResolvedCapability(c), ChannelID: mt.Channel.ID, BaseURL: mt.Channel.BaseURL, GroupID: group.ID, TokenID: token.ID, TokenName: token.Name, PricingCurrency: currency, PricingVersion: version, Price: price, DisableResolutionFuzzyMatch: mc.DisableResolutionFuzzyMatch}
	...
	task, created, err := model.ReserveImageTask(&model.ImageTask{ID: id, Model: wrapper.ID, RequestModel: middleware.GetRequestedModel(c), ValidationContract: string(rawWrapper.Contract), GroupID: group.ID, TokenID: token.ID, Fingerprint: fingerprint, UpstreamModel: mt.ActualModel, ChannelType: mt.Channel.Type, KeyFingerprint: model.ImageChannelKeyFingerprint(mt.Channel.Key), ExpectedImages: input.N}, info, middleware.OperationalFieldsFromContext(c))
	...
	if err != nil {
		...
		if model.SetImageTaskResult(id, status, nil, taskError) != nil {
			imageTaskHTTPError(c, 503, "task_store_unavailable")
			return
		}
		task.Status = status
	...
	if err = model.AcceptImageTask(id, upstream); err != nil {
		imageTaskHTTPError(c, 503, "task_store_unavailable")
		return
	}
	task.Status = "queued"
...
func replayImageTask(c *gin.Context) {
	...
	normalized, validationErr := registryvalidation.ValidateImage([]byte(task.ValidationContract), contract.ID, body)

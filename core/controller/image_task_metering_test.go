...
func TestImageTaskMeasuredAdmissionBeforePaidSubmit(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ch := &model.Channel{ID: 7, Type: model.ChannelTypeFal, Key: "secret", BaseURL: upstreamServer.URL, Models: []string{"image"}, ModelMapping: map[string]string{"image": "fal-ai/test"}, Configs: map[string]any{providerBindingsConfig: map[string]any{"image": binding}}}
			max := int64(100)
			price := model.Price{ImageBilling: &model.ImageBillingPolicy{Version: 1, Scenario: "generation", Input: &model.ImageBillingInput{ChargeBasis: "per_output", FirstNFree: 1, AmountMicros: 50000, UnitQuantity: 1}, OutputPixelTiers: []model.ImageBillingTier{{MaxPixels: &max, AmountMicros: 400000, UnitQuantity: 1}, {AmountMicros: 200000, UnitQuantity: 1}}}}
			if tc.conditional {
			...
			for run := 0; run < 2; run++ {
				...
				c.Request = httptest.NewRequest(http.MethodPost, "/v1/images/tasks", bytes.NewBufferString(body))
				...
				c.Set(middleware.ModelConfig, model.ModelConfig{Price: price, Config: map[model.ModelConfigKey]any{"x_token_platform_capability_contract": map[string]any{"entry_id": "image", "contract": contract}, "x_token_platform_pricing": map[string]any{"currency": "USD", "pricing_version": "v1"}}})
				c.Set(middleware.GroupBalance, &middleware.GroupBalanceConsumer{Group: "g", CheckBalance: func(required float64) bool {
					if textOnly {

...
func TestImageTaskHTTPReplayConflictAndOwner(t *testing.T) {
	...
	_, _, err = model.ReserveImageTask(&model.ImageTask{ID: "request-a", Model: "public", GroupID: "g", TokenID: 1, Fingerprint: fp}, &model.AsyncUsageInfo{})
	require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		c.Request = httptest.NewRequest(tc.method, "/v1/images/tasks/request-a", strings.NewReader(tc.body))
		...
		c.Set(middleware.ModelConfig, model.ModelConfig{Config: map[model.ModelConfigKey]any{"x_token_platform_capability_contract": map[string]any{"contract": map[string]any{"execution": map[string]any{"mode": "async", "output": "image"}}}}})
		if tc.method == "POST" {
			submitImageTask(c)
		} else {
			GetImageTask(c)
...
func TestImageReplayDoesNotRequireActivePricingOrChannel(t *testing.T) {
	...
	require.NoError(t, db.Create(&model.ImageTask{ID: "replay-id", Model: "public", RequestModel: "public", GroupID: "g", TokenID: 1, Fingerprint: fp, Status: "completed", ValidationContract: contract}).Error)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("POST", "/v1/images/tasks", strings.NewReader(`{"model":"public","prompt":"test"}`))
...
func TestImageSubmitWithCompiledRegistryFixture(t *testing.T) {
	...
	contract, _ := json.Marshal(metadata["contract"])
	const public = "minimax/image-01/text-to-image"
	normalized, validationErr := registryvalidation.ValidateImage(contract, public, []byte(`{"model":"minimax/image-01/text-to-image","prompt":"test"}`))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				require.Equal(t, map[string]any{"prompt": "test", "num_images": float64(1), "aspect_ratio": "1:1", "prompt_optimizer": false}, body)
				...
				w.WriteHeader(code)
				if code == 200 {
					w.Write([]byte(`{"request_id":"upstream"}`))
				}
			}))
			defer s.Close()
			ch := &model.Channel{ID: 7, Type: model.ChannelTypeFal, Key: "secret", BaseURL: s.URL, Models: []string{public}, ModelMapping: map[string]string{public: "fal-ai/minimax/image-01"}}
			w := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(w)
			c.Request = httptest.NewRequest("POST", "/v1/images/tasks", bytes.NewReader(normalized))
			...
			c.Set(middleware.ModelConfig, model.ModelConfig{Price: tc.price, Config: map[model.ModelConfigKey]any{"x_token_platform_capability_contract": metadata, "x_token_platform_pricing": map[string]any{"currency": "USD", "pricing_version": "fixture-v1"}}})
			balanceChecks := 0

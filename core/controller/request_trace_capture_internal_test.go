...
func TestRelayHelperTracesEachActualAttemptWithoutOverwritingPreviousAttempt(t *testing.T) {
	...
	failure := relaymodel.WrapperOpenAIErrorWithMessage(
		"secret upstream failure",
		"upstream_failed",
		http.StatusBadGateway,
	)

	result, _ := RelayHelper(
		c,
		first,
...
func TestRelayHelperClassifiesContextAndPanicOutcomes(t *testing.T) {
	t.Run("timeout", func(t *testing.T) {
		...
		require.Equal(
			t,
			requesttrace.StatusTimeout,
			terminalCaptureSpans(*spans, requesttrace.StageUpstreamAttempt)[0].Status,
		)
	})
...
func TestValidateRelayRequestTracesOnlyAnActualValidatorCall(t *testing.T) {
	...
	require.ErrorIs(
		t,
		validateRelayRequest(c, mc, func(*gin.Context, model.ModelConfig) error { return want }),
		want,
	)
	finish(requesttrace.StatusError)
...
func TestAttemptAttributesOmitSyntheticDefaultConfigModel(t *testing.T) {
	...
	m := meta.NewMeta(
		...
		attackerModel,
		model.NewDefaultModelConfig(attackerModel),
	)
	attrs := requestTraceAttemptAttributes(m, 1)
	require.Empty(t, attrs.PublicModelID)
	require.Empty(t, attrs.UpstreamModelID)
...
func TestAttemptAttributesUseValidatedCapabilityPublicIdentity(t *testing.T) {
	...
	t.Run("image capability uses validated public capability ID", func(t *testing.T) {
		...
		m := meta.NewMeta(
			...
			mc,
			meta.WithRoutingModel(mc.Model),
		)
		attrs := requestTraceAttemptAttributes(m, 1)
...
func captureTestMeta(channelID int, mapped string) *meta.Meta {
	m := meta.NewMeta(
		...
		mode.ImagesGenerations,
		"attacker-input",
		model.NewDefaultModelConfig("public-model"),
		meta.WithRoutingModel("public-model"),
	)
	m.SetChannel(
		&model.Channel{ID: channelID, ModelMapping: map[string]string{"public-model": mapped}},
	)

	return m
}

func newCaptureTestContext(
	t *testing.T,
	ctx context.Context,
) (*gin.Context, *[]requesttrace.Span, func(requesttrace.Status)) {
	t.Helper()

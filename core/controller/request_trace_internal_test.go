...
func TestGetRequestTraceRejectsMissingHeaderAndInvalidParameters(t *testing.T) {
	...
	cases := []string{
		...
		fmt.Sprintf(
			"/api/trace/group-one/%s?service=aiproxy",
			strings.ToUpper(controllerTraceID(10)),
		),
		fmt.Sprintf(
			"/api/trace/group-one/%s?service=aiproxy&after=%s",
			controllerTraceID(1),
			strings.ToUpper(controllerTraceID(10)),
		),
...
func controllerTraceSpan(sequence int) requesttrace.Span {
	...
	return requesttrace.Span{
		...
		StartedAt: time.Date(2026, time.September, 8, 1, 2, 3, 0, time.UTC), Revision: 1,
	}
}

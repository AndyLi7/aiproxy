...
func TestAsyncUsageCapabilitySurvivesRetryAndCompletion(t *testing.T) {
	for _, capability := range []string{string(ModelCapabilityTextToVideo), string(ModelCapabilityImageToVideo), "reference-to-video"} {
		t.Run(capability, func(t *testing.T) {
			...
			require.Equal(t, capability, retried.Capability)
			...
			require.Equal(t, capability, got.Capability)
		})
	}
}

...
func ResolveImageCapabilityRoute(
	publicID string,
	lookup func(string) (map[ModelConfigKey]any, bool),
) (string, string) {
	...
	if !valid || version != ModelCapabilityContractVersion || config["public_model"] != parent ||
		config["capability"] != capability ||
		config["public_capability_model"] != publicID {
		return publicID, ""
	}

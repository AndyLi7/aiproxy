...
func GetImageTask(id, group string, token int) (*ImageTask, error) {
	var task ImageTask
	err := LogDB.Where("id = ? AND group_id = ? AND token_id = ?", id, group, token).First(&task).Error
	return &task, err
}
func ReserveImageTask(task *ImageTask, info *AsyncUsageInfo, operational ...OperationalFields) (*ImageTask, bool, error) {
	created := false
	err := LogDB.Transaction(func(tx *gorm.DB) error {
		...
		if result.RowsAffected == 0 {
			...
			if existing.GroupID != task.GroupID || existing.TokenID != task.TokenID || existing.Model != task.Model || existing.Fingerprint != task.Fingerprint {
		...
		var prior int64
		if err := tx.Model(&Log{}).Where("request_id = ?", task.ID).Count(&prior).Error; err != nil {
			return err
		}
		if prior > 0 {
			return ErrImageTaskConflict
		}
		entry := &Log{RequestID: EmptyNullString(task.ID), RequestAt: info.RequestAt, GroupID: info.GroupID, TokenID: info.TokenID, TokenName: info.TokenName, Model: info.Model, ChannelID: info.ChannelID, Mode: info.Mode, Code: 202, Endpoint: "/v1/images/tasks", RequestSource: RequestSourceAPI, Currency: info.PricingCurrency, PricingVersion: info.PricingVersion, Price: info.Price, AsyncUsageStatus: AsyncUsageStatusPending}
		if len(operational) > 0 {
			fields := operational[0]
			entry.RequestSource = fields.RequestSource
			entry.Model, entry.Capability = PublicLogIdentity(info.Model, fields.PublicModel, fields.ResolvedCapability)
		}
...
func AcceptImageTask(id, upstream string) error {
	return LogDB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&task).Updates(map[string]any{"status": "queued", "upstream_id": upstream}).Error; err != nil {
			return err
		}
		if err := tx.Model(&Log{}).Where("request_id = ?", id).Update("upstream_id", upstream).Error; err != nil {
			return err
		}
		return tx.Model(&AsyncUsageInfo{}).Where("id = ?", task.UsageID).Updates(map[string]any{"status": AsyncUsageStatusPending, "upstream_id": upstream, "next_poll_at": time.Now()}).Error
	})
}
func SetImageTaskResult(id, status string, data []ImageOutput, taskError *ImageTaskError) error {
	...
	return LogDB.Transaction(func(tx *gorm.DB) error {
		result := tx.Model(&ImageTask{}).Where("id = ? AND status NOT IN ?", id, []string{"completed", "failed"}).Updates(map[string]any{"status": status, "data": mustImageJSON(data), "error": mustImageJSON(taskError), "updated_at": time.Now()})
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected == 0 {
		...
		if status == "failed" {
			if err := tx.Model(&AsyncUsageInfo{}).Where("image_task_id = ?", id).Update("status", AsyncUsageStatusFailed).Error; err != nil {
				return err
			}
			return tx.Model(&Log{}).Where("request_id = ?", id).Updates(map[string]any{"async_usage_status": AsyncUsageStatusFailed, "code": 502, "safe_error": "Image generation failed"}).Error
		}
		return nil
	})
}

func mustImageJSON(v any) string { b, _ := json.Marshal(v); return string(b) }
...
func CheckImageChannelRetention(ids []int) error {
	...
	var count int64
	err := LogDB.Model(&AsyncUsageInfo{}).Where("channel_id IN ? AND image_task_id <> ? AND status IN ?", ids, "", []AsyncUsageStatus{AsyncUsageStatusNone, AsyncUsageStatusPending}).Count(&count).Error
	if err != nil {
		return err
	}
	if count > 0 {
...
func preserveImageAccountingLogs(tx *gorm.DB) *gorm.DB {
	...
	return tx.Where("id NOT IN (?)", LogDB.Model(&AsyncUsageInfo{}).Select("log_id").Where("image_task_id <> ? AND status IN ?", "", []AsyncUsageStatus{AsyncUsageStatusNone, AsyncUsageStatusPending}))
}
func RecoverStaleImageSubmissions(now time.Time) error {
	return LogDB.Model(&ImageTask{}).Where("status = ? AND updated_at < ?", "submitting", now.Add(-2*time.Minute)).Update("status", "submission_unknown").Error
}

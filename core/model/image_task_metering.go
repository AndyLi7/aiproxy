...
func QueueImageMaximumAmount(p Price, outputs int, inputs int64) (float64, error) {
	if p.ImageBilling == nil || p.ImageBilling.Scenario != "generation" || len(p.ConditionalPrices) != 0 || outputs < 1 || outputs > ImageBillingMaxOutputs || inputs < 0 {
	...
	var maximum int64
	for _, r := range rates {
		q := new(big.Int).Add(big.NewInt(r.AmountMicros), new(big.Int).Sub(big.NewInt(r.UnitQuantity), big.NewInt(1)))
	...
	usage := &ImageUsage{Version: 1, State: "complete", Scenario: "generation", InputCount: &inputs, Outputs: make([]ImageUsageOutput, outputs)}

...
type RequestTraceHead struct {
	TraceID   string               `json:"TraceID"   gorm:"primaryKey;size:32"`
	Service   requesttrace.Service `json:"Service"   gorm:"primaryKey;size:16"`
	GroupID   string               `json:"GroupID"   gorm:"size:64;index"`
	SpanCount int                  `json:"SpanCount"`
	Truncated bool                 `json:"Truncated"`
	UpdatedAt time.Time            `json:"UpdatedAt" gorm:"index:idx_request_trace_head_updated_at"`
}
...
type RequestTraceSpan struct {
	SpanID       string                  `json:"SpanID"       gorm:"primaryKey;size:32;index:idx_request_trace_scope,priority:4;index:idx_request_trace_request_scope,priority:5"`
	Version      int                     `json:"Version"`
	TraceID      string                  `json:"TraceID"      gorm:"size:32;index:idx_request_trace_scope,priority:2"`
	Service      requesttrace.Service    `json:"Service"      gorm:"size:16;index:idx_request_trace_scope,priority:3;index:idx_request_trace_request_scope,priority:3"`
	GroupID      string                  `json:"GroupID"      gorm:"size:64;index:idx_request_trace_scope,priority:1;index:idx_request_trace_request_scope,priority:1"`
	RequestID    string                  `json:"RequestID"    gorm:"size:128;index;index:idx_request_trace_request_scope,priority:2"`
	ParentSpanID string                  `json:"ParentSpanID" gorm:"size:32"`
	Stage        requesttrace.Stage      `json:"Stage"        gorm:"size:32;index:idx_request_trace_request_scope,priority:4"`
	Status       requesttrace.Status     `json:"Status"       gorm:"size:16;index"`
	StartedAt    time.Time               `json:"StartedAt"    gorm:"index"`
	EndedAt      *time.Time              `json:"EndedAt"`
	DurationMS   *float64                `json:"DurationMS"`
	Revision     int                     `json:"Revision"`
	Attributes   requesttrace.Attributes `json:"Attributes"   gorm:"serializer:json;type:text"`
...
func (s *TraceStore) Migrate(ctx context.Context) error {
	...
	return s.db.WithContext(ctx).
		AutoMigrate(&RequestTraceHead{}, &RequestTraceSpan{}, &RequestTraceNonce{}, &RequestTraceTask{})
}

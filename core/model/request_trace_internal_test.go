...
func TestTraceStoreIdempotentReplayDoesNotRefreshRetentionTimestamp(t *testing.T) {
	...
	require.True(
		t,
		persisted.UpdatedAt.Equal(old),
		"span replay refreshed updated_at: %v",
		persisted.UpdatedAt,
	)
	...
	require.True(
		t,
		head.UpdatedAt.Equal(old),
		"head replay refreshed updated_at: %v",
		head.UpdatedAt,
	)
...
func traceTestSpan(sequence int) requesttrace.Span {
	return requesttrace.Span{
		...
		StartedAt:    time.Date(2026, time.September, 8, 1, 2, sequence%60, 0, time.UTC),
		Revision:     1,
	}
}

...
func (s *TraceStore) ClaimTraceNonce(
	ctx context.Context,
	digest string,
	expires time.Time,
) (bool, error) {
	...
	if err != nil || len(decoded) != 32 || strings.ToLower(digest) != digest ||
		!expires.After(now) ||
		expires.After(now.Add(3*time.Minute)) {
		return false, errors.New("invalid trace nonce claim")
	}

	result := s.db.WithContext(ctx).
		Clauses(clause.OnConflict{DoNothing: true}).
		Create(&RequestTraceNonce{Digest: digest, ExpiresAt: expires.UTC()})

	return result.Error == nil && result.RowsAffected == 1, result.Error
}

func (s *TraceStore) CleanTraceNonces(
	ctx context.Context,
	now time.Time,
	limit int,
) (int64, error) {
	...
	if err := s.db.WithContext(ctx).
		Model(&RequestTraceNonce{}).
		Where("expires_at < ?", now.UTC()).
		Order("expires_at, digest").
		Limit(limit).
		Pluck("digest", &digests).
		Error; err != nil {
		return 0, err
	}
	...
	result := s.db.WithContext(ctx).
		Where("digest IN ? AND expires_at < ?", digests, now.UTC()).
		Delete(&RequestTraceNonce{})

...
func TestTraceStorePostgresMatchesLifecycleAfterTimestampPrecisionRoundTrip(t *testing.T) {
	...
	started.StartedAt = time.Date(2026, time.September, 8, 1, 2, 3, 123456789, time.UTC)
...
func TestTraceStorePostgresSerializesConcurrent256thAnd257thSpan(t *testing.T) {
	...
	require.NoError(
		t,
		db.First(&head, "trace_id = ? AND service = ?", traceTestID(999), "aiproxy").Error,
	)
...
func TestTraceStorePostgresCleanupRechecksCutoffAndCreatesTraceIndexes(t *testing.T) {
	...
	require.True(
		t,
		db.Migrator().HasIndex(&RequestTraceHead{}, "idx_request_trace_head_updated_at"),
	)

	var scopeIndex string
	require.NoError(
		t,
		db.Raw("SELECT indexdef FROM pg_indexes WHERE schemaname = current_schema() AND indexname = ?", "idx_request_trace_scope").
			Scan(&scopeIndex).
			Error,
	)
	...
	require.NoError(
		t,
		db.Raw("SELECT indexdef FROM pg_indexes WHERE schemaname = current_schema() AND indexname = ?", "idx_request_trace_request_scope").
			Scan(&requestScopeIndex).
			Error,
	)
	...
	require.NoError(t, db.Callback().
		Delete().
		Before("gorm:delete").
		Register("request_trace_cleanup_refresh", func(tx *gorm.DB) {
...
func TestTraceStorePostgresCleanupDoesNotOrphanConcurrentWriter(t *testing.T) {
	...
	require.NoError(
		t,
		db.Model(&RequestTraceHead{}).
			Where("trace_id = ? AND service = ?", expired.TraceID, expired.Service).
			UpdateColumn("updated_at", old).
			Error,
	)
	...
	require.NoError(t, db.Callback().
		Delete().
		Before("gorm:delete").
		Register("request_trace_cleanup_concurrent_writer", func(tx *gorm.DB) {
			if tx.Statement.Schema == nil || tx.Statement.Schema.Name != "RequestTraceSpan" {
				return
			}
	...
	require.NoError(
		t,
		db.First(&head, "trace_id = ? AND service = ?", writer.TraceID, writer.Service).Error,
	)
	...
	require.NoError(
		t,
		db.Where("trace_id = ? AND service = ?", writer.TraceID, writer.Service).Find(&spans).Error,
	)

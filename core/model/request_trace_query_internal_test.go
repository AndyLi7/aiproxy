...
func TestTraceStoreListScopesByGroupTraceAndServiceAndPaginatesBySpanID(t *testing.T) {
	...
	require.NoError(t, db.Create(&RequestTraceSpan{
		...
		StartedAt: time.Date(2026, time.September, 8, 1, 2, 7, 0, time.UTC),
		Revision:  1,
	}).Error)
...
func TestTraceStoreListUsesDefaultAndMaximumLimitsAndReturnsDatabaseErrors(t *testing.T) {
	...
	_, err = store.List(
		ctx,
		TraceQuery{GroupID: "group-one", TraceID: traceID, Service: requesttrace.ServiceAIProxy},
	)
...
func TestTraceStoreFindRequestsScopesExactRootRequestsAndPaginatesCandidates(t *testing.T) {
	...
	fixtures := []RequestTraceSpan{
		{
			SpanID:    traceTestID(901),
			TraceID:   traceTestID(801),
			GroupID:   "group-a",
		...
		{
			SpanID:    traceTestID(902),
			TraceID:   traceTestID(802),
			GroupID:   "group-a",
			RequestID: "reused-request",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageRequest,
		...
		{
			SpanID:    traceTestID(903),
			TraceID:   traceTestID(803),
			GroupID:   "group-b",
			RequestID: "reused-request",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageRequest,
		...
		{
			SpanID:    traceTestID(904),
			TraceID:   traceTestID(804),
			GroupID:   "group-a",
			RequestID: "reused-request",
			Service:   requesttrace.ServiceApp,
			Stage:     requesttrace.StageRequest,
			Status:    requesttrace.StatusSuccess,
			StartedAt: now,
			UpdatedAt: now,
		...
		{
			SpanID:    traceTestID(905),
			TraceID:   traceTestID(805),
			GroupID:   "group-a",
			RequestID: "reused-request",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageGatewayCall,
			Status:    requesttrace.StatusSuccess,
			StartedAt: now,
			UpdatedAt: now,
		},
		{
			SpanID:       traceTestID(906),
			TraceID:      traceTestID(806),
			GroupID:      "group-a",
			RequestID:    "reused-request",
			ParentSpanID: traceTestID(1),
			Service:      requesttrace.ServiceAIProxy,
			Stage:        requesttrace.StageRequest,
			Status:       requesttrace.StatusSuccess,
			StartedAt:    now,
		...
		{
			SpanID:    traceTestID(907),
			TraceID:   traceTestID(807),
			GroupID:   "group-a",
			RequestID: "reused-request",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageRequest,
	...
	first, err := store.FindRequests(
		context.Background(),
		TraceRequestQuery{GroupID: "group-a", RequestID: "reused-request", Limit: 1},
	)
	require.NoError(t, err)
	require.Equal(
		t,
		[]TraceRequestCandidate{
			{
				TraceID:   traceTestID(801),
				SpanID:    traceTestID(901),
				StartedAt: fixtures[0].StartedAt,
				Status:    requesttrace.StatusSuccess,
			},
		},
		first.Items,
	...
	second, err := store.FindRequests(
		context.Background(),
		TraceRequestQuery{
			GroupID:     "group-a",
			RequestID:   "reused-request",
			AfterSpanID: first.NextCursor,
			Limit:       1,
		},
	)
	require.NoError(t, err)
		...
		[]TraceRequestCandidate{
			{
				TraceID:   traceTestID(802),
				SpanID:    traceTestID(902),
				StartedAt: fixtures[1].StartedAt,
				Status:    requesttrace.StatusError,
			},
		},
		second.Items,
	...
	other, err := store.FindRequests(
		context.Background(),
		TraceRequestQuery{GroupID: "group-b", RequestID: "reused-request"},
	)
	require.NoError(t, err)
...
func TestTraceStoreFindRequestsRejectsUnscopedQueriesAndBoundsLimit(t *testing.T) {
	...
	for i := range fixtures {
		fixtures[i] = RequestTraceSpan{
			SpanID:    traceTestID(1100 + i),
			TraceID:   traceTestID(1000 + i),
			GroupID:   "group",
			RequestID: "request",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageRequest,
	...
	defaultPage, err := store.FindRequests(
		context.Background(),
		TraceRequestQuery{GroupID: "group", RequestID: "request"},
	)
	require.NoError(t, err)
	require.Len(t, defaultPage.Items, 50)
	require.Equal(t, fixtures[49].SpanID, defaultPage.NextCursor)

	maximumPage, err := store.FindRequests(
		context.Background(),
		TraceRequestQuery{GroupID: "group", RequestID: "request", Limit: 1000},
	)
	require.NoError(t, err)
	...
	_, err = NewTraceStore(
		nil,
	).FindRequests(context.Background(), TraceRequestQuery{GroupID: "group", RequestID: "request"})
	require.Error(t, err)
}
...
func TestTraceStoreCleanExpiredDeletesOnlyExpiredTraceRowsInBoundedBatches(t *testing.T) {
	...
	require.NoError(
		t,
		db.Create(
			&Token{GroupID: "group-one", Name: "retained-quota", UsedAmount: 12.5, Quota: 50},
		).Error,
	)
	...
	for _, fixture := range []struct {
		...
		require.NoError(
			t,
			db.Model(&RequestTraceSpan{}).
				Where("span_id = ?", span.SpanID).
				UpdateColumn("updated_at", fixture.updated).
				Error,
		)
		require.NoError(
			t,
			db.Model(&RequestTraceHead{}).

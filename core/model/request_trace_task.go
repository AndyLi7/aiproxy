...
func (s *TraceStore) TaskTraceSummary(
	ctx context.Context,
	group, traceID string,
	now time.Time,
) (*TraceTaskSummary, error) {
	...
	err := s.db.WithContext(ctx).
		Select("poll_count, first_poll_at, last_poll_at").
		Where("group_id = ? AND trace_id = ? AND expires_at > ?", group, traceID, now.UTC()).
		Limit(2).
		Find(&rows).
		Error
	if err != nil {
	...
	return &TraceTaskSummary{
		PollCount:   rows[0].PollCount,
		FirstPollAt: rows[0].FirstPollAt,
		LastPollAt:  rows[0].LastPollAt,
	}, nil
}

func (s *TraceStore) SaveTaskTrace(
	ctx context.Context,
	asyncID int,
	group, traceID, parentID string,
) error {
	if s == nil || s.db == nil || ctx == nil || asyncID <= 0 || group == "" ||
		!traceTaskRandomID(traceID) ||
		!traceTaskRandomID(parentID) {
		return errors.New("trace task association unavailable")
	}

	var info AsyncUsageInfo
	if err := s.db.WithContext(ctx).
		Where("id = ? AND group_id = ?", asyncID, group).
		First(&info).
		Error; err != nil {
		return err
	}

	if info.TokenID <= 0 || info.ChannelID <= 0 || info.UpstreamID == "" ||
		len(info.UpstreamID) > 256 {
		return errors.New("trace task identity unavailable")
	}

	link := RequestTraceTask{
		AsyncUsageID: info.ID,
		GroupID:      info.GroupID,
		TokenID:      info.TokenID,
		ChannelID:    info.ChannelID,
		TaskID:       info.UpstreamID,
		TraceID:      traceID,
...
func (s *TraceStore) ResolveTaskTrace(
	ctx context.Context,
	group string,
	tokenID, channelID int,
	taskID string,
	now time.Time,
) (RequestTraceTask, error) {
	if s == nil || s.db == nil || ctx == nil || group == "" || tokenID <= 0 || channelID <= 0 ||
		taskID == "" ||
		len(taskID) > 256 ||
		now.IsZero() {
	...
	err := s.db.WithContext(ctx).
		Where("group_id = ? AND token_id = ? AND channel_id = ? AND task_id = ? AND expires_at > ?", group, tokenID, channelID, taskID, now.UTC()).
		Limit(2).
		Find(&links).
		Error
	if err != nil {
...
func (s *TraceStore) RecordTaskPoll(
	ctx context.Context,
	id int,
	group string,
	observed time.Time,
	...
	result := s.db.WithContext(ctx).
		Model(&RequestTraceTask{}).
		Where("async_usage_id = ? AND group_id = ?", id, group).
		Updates(map[string]any{
			"poll_count": gorm.Expr(
				"CASE WHEN poll_count < 9223372036854775807 THEN poll_count + 1 ELSE poll_count END",
			),
			"first_poll_at": gorm.Expr(
				"CASE WHEN first_poll_at IS NULL OR first_poll_at > ? THEN ? ELSE first_poll_at END",
				at,
				at,
			),
			"last_poll_at": gorm.Expr(
				"CASE WHEN last_poll_at IS NULL OR last_poll_at < ? THEN ? ELSE last_poll_at END",
				at,
				at,
			),
			"expires_at": gorm.Expr(
				"CASE WHEN expires_at < ? THEN ? ELSE expires_at END",
				at.Add(14*24*time.Hour),
				at.Add(14*24*time.Hour),
			),
		})
	if result.Error != nil {
...
func (s *TraceStore) CleanTaskTraces(ctx context.Context, now time.Time, limit int) (int64, error) {
	...
	if err := s.db.WithContext(ctx).
		...
		Order("expires_at, async_usage_id").
		Limit(limit).
		Pluck("async_usage_id", &ids).
		Error; err != nil {
		return 0, err
	}
	...
	result := s.db.WithContext(ctx).
		Where("async_usage_id IN ? AND expires_at < ?", ids, now.UTC()).
		Delete(&RequestTraceTask{})

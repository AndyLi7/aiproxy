...
func TestTaskTracePersistsAndResolvesFromStoredTaskIdentity(t *testing.T) {
	...
	link, err := resumed.ResolveTaskTrace(
		context.Background(),
		"group-a",
		7,
		9,
...
func TestTaskTraceCleanupRemovesOnlyExpiredAssociations(t *testing.T) {
	...
	require.NoError(
		t,
		db.Create(&RequestTraceTask{AsyncUsageID: 1, ExpiresAt: now.Add(-time.Hour)}).Error,
	)
	require.NoError(
		t,
		db.Create(&RequestTraceTask{AsyncUsageID: 2, ExpiresAt: now.Add(time.Hour)}).Error,
	)
...
func TestTaskTracePollSummarySurvivesDetailedSpanLimit(t *testing.T) {
	...
	require.NoError(
		t,
		db.Create(
			&RequestTraceTask{AsyncUsageID: 1, GroupID: "group-a", ExpiresAt: now.Add(time.Hour)},
		).Error,
	)
	...
	for i := range 260 {
		require.NoError(
			t,
			store.RecordTaskPoll(ctx, 1, "group-a", now.Add(time.Duration(i)*time.Second)),
		)
	}

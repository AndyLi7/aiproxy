...
func TestVerifiedVideoBillableDimensionsRequireTokenFormulaMatch(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "overflow alias token count",
			size:         "854x480",
			seconds:      2,
			outputTokens: 19845 + (1 << 54),
			wantOK:       false,
		},
		{
			name:         "dimension above safety bound",
			size:         "16385x1024",
			seconds:      2,
			outputTokens: 803600,
			wantOK:       false,
		...
		{
			name:         "duration above safety bound",
			size:         "1024x1024",
			seconds:      3601,
			outputTokens: 88499200,
			wantOK:       false,
		},
	}

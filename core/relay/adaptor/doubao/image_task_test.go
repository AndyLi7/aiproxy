...
func syncContract() []byte {
	return []byte(`{"provider_contract_version":1,"selected_provider_binding":{"provider":"ark","id":"ark","revision":"1","contractHash":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},"providers":{"ark":{"upstream":{"id":"ark","revision":"1","contractHash":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","endpoint":"seedream-test","outputJsonSchema":{"type":"object","required":["data"],"properties":{"data":{"type":"array","minItems":1}}}}}}}`)
}
func TestSyncImageBridgeOneRequest(t *testing.T) {
	...
	defer s.Close()
	m := &meta.Meta{ActualModel: "seedream-test", Channel: meta.ChannelMeta{BaseURL: s.URL + "/api/v3", Key: "secret"}}
	got, err := (&Adaptor{}).GenerateImage(t.Context(), m, []byte(`{"prompt":"hello","stream":false,"response_format":"url"}`), syncContract())

...
func parseSeedance25ReferenceRequest(req *http.Request) (doubaoVideoRequest, error) {
	...
	if strings.TrimSpace(prompt) == "" || len(utf16.Encode([]rune(prompt))) > 2000 {
		return doubaoVideoRequest{}, errors.New("prompt must contain 1 to 2000 characters")
	}

	if task != "reference" && task != "edit" && task != "extend" {
		return doubaoVideoRequest{}, errors.New("invalid reference task type")
	}

	if seconds != -1 && (seconds < 4 || seconds > 30) {
		return doubaoVideoRequest{}, errors.New("reference duration must be -1 or 4 to 30 seconds")
	}

	if task == "edit" && seconds != -1 {
		return doubaoVideoRequest{}, errors.New("editing requires automatic duration")
	}

	if resolution != "480p" && resolution != "720p" && resolution != "1080p" {
		return doubaoVideoRequest{}, errors.New("unsupported reference resolution")
	}
	...
	if !validRatios[ratio] || (task != "reference" && ratio != "adaptive") {
		return doubaoVideoRequest{}, errors.New("invalid aspect ratio for reference task")
	}
	...
	return appendSeedance25References(request, raw)
}

func appendSeedance25References(
	request doubaoVideoRequest,
	...
	for _, field := range []struct {
		...
		for _, ref := range refs {
			...
			if err != nil || parsed.Scheme != "https" || parsed.Hostname() == "" {
				return doubaoVideoRequest{}, fmt.Errorf(
					"%s references require HTTPS URLs",
					field.key,
				)
			}
	...
	if total == 0 || total > 50 {
		return doubaoVideoRequest{}, errors.New("provide 1 to 50 references")
	}

	if request.OmniReferenceTaskType != "reference" && videos == 0 {
		return doubaoVideoRequest{}, errors.New("editing and extension require a reference video")
	}

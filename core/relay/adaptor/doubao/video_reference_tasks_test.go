...
func referenceTaskMeta() *relaymeta.Meta {
	m := relaymeta.NewMeta(nil, mode.Videos, "bytedance/seedance-2.5/reference-to-video", coremodel.ModelConfig{
		Config: map[coremodel.ModelConfigKey]any{
			...
			coremodel.ModelConfigParameterSchemaKey:           map[string]any{"type": "object", "properties": map[string]any{}},
...
func TestReferenceTaskConvertsPublicFields(t *testing.T) {
	for _, task := range []string{"reference", "edit", "extend"} {
		t.Run(task, func(t *testing.T) {
			payload := map[string]any{"prompt": "Change this scene", "task_type": task, "seconds": -1, "aspect_ratio": "adaptive", "video_urls": []string{"https://example.com/source.mp4"}, "image_urls": []string{"https://example.com/ref.png"}, "audio_urls": []string{"https://example.com/ref.wav"}}
			data, err := json.Marshal(payload)
			require.NoError(t, err)
			req := httptest.NewRequest(http.MethodPost, "/v1/videos", bytes.NewReader(data))
...
func TestReferenceTaskRejectsInvalidPublicParameters(t *testing.T) {
	for _, payload := range []string{
		...
		req := httptest.NewRequest(http.MethodPost, "/v1/videos", bytes.NewBufferString(payload))
		req.Header.Set("Content-Type", "application/json")
...
func TestReferenceTaskSupportsAudioOnlyAndRejectsTooManyReferences(t *testing.T) {
	for _, count := range []int{1, 10, 11} {
		...
		require.NoError(t, err)
		req := httptest.NewRequest(http.MethodPost, "/v1/videos", bytes.NewReader(payload))
		req.Header.Set("Content-Type", "application/json")
		_, err = ConvertVideosRequest(referenceTaskMeta(), req)
		if count > 10 {
			require.Error(t, err)

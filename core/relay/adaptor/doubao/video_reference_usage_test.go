...
func TestReferenceResponsePreservesFractionalSeconds(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal([]byte(`{"id":"task-1","status":"succeeded","duration":4.6,"usage":{"completion_tokens":1234}}`), &response))
	...
	require.Equal(t, coremodel.ZeroNullInt64(1234), doubaoVideoUsageToModelUsage(response.Usage).OutputTokens)
}
...
func TestReferenceMissingUsageRemainsRetryable(t *testing.T) {
	...
	defer server.Close()
	store := &doubaoTestStore{saved: []adaptor.StoreCache{{ID: coremodel.VideoGenerationStoreID("task-1"), Metadata: `{"reference_task_type":"edit","duration":-1}`}}}

...
type ImageTaskAdapter interface {
	...
	SubmitImage(context.Context, *meta.Meta, []byte) (string, error)
	PollImage(context.Context, *model.Channel, *model.AsyncUsageInfo, *model.ImageTask) (ImageTaskResult, error)
}
...
func MapImageProviderInput(config map[model.ModelConfigKey]any, adapterName string, body []byte) ([]byte, error) {
	var wrapper struct {
		Contract struct {

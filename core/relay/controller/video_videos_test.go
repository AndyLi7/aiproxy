...
func TestValidateVideosRequestRejectsFuzzySizeWhenCapabilitiesArePublished(t *testing.T) {
	...
	require.Equal(
		t,
		[]string{"854x480", "480x854", "480x480", "1280x720", "720x1280", "720x720"},
		paramErr.AllowedValues,
	)
}
...
func TestValidateVideosRequestRejectsFixedSizeForAdaptiveOnlyModel(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(
		...
		"/v1/videos",
		bytes.NewBufferString(
			`{"model":"video-model","prompt":"A city street","size":"1280x720","seconds":5}`,
		),
	)
	req.Header.Set("Content-Type", "application/json")
...
func TestValidateVideosRequestRequiresPrompt(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(
		...
		http.MethodPost,
		"/v1/videos",
		bytes.NewBufferString(`{"model":"video-model"}`),
	)
	req.Header.Set("Content-Type", "application/json")

...
func TestRequestTraceCaptureRealGatewayRoutes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			body: func() (*bytes.Buffer, string) {
				return bytes.NewBufferString(
					`{"model":"trace-image","prompt":"private prompt"}`,
				), "application/json"
			},
			wantStages: []requesttrace.Stage{
		...
		},
		{
			...
			body: func() (*bytes.Buffer, string) {
				return bytes.NewBufferString(
					`{"model":"trace-video","capability":"text-to-video","prompt":"private video prompt","size":"480x480","seconds":4}`,
				), "application/json"
			},
			wantStages: []requesttrace.Stage{
				requesttrace.StageRequest,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			page, err := fixture.store.FindRequests(
				t.Context(),
				model.TraceRequestQuery{GroupID: "group-a", RequestID: requestID},
			)
			require.NoError(t, err)
			require.Len(t, page.Items, 1)
			spans, err := fixture.store.List(
				t.Context(),
				model.TraceQuery{
					GroupID: "group-a",
					TraceID: page.Items[0].TraceID,
...
func assertExactTraceLifecycle(
	t *testing.T,
	spans []requesttrace.Span,
	stages []requesttrace.Stage,
) {
	t.Helper()
	...
	for _, stage := range stages[1:] {
		...
		if previous != nil {
			require.False(
				t,
				span.StartedAt.Before(previous.StartedAt),
				"%s started before %s",
				stage,
				previous.Stage,
			)
...
func assertExactModelTraceLifecycle(
	t *testing.T,
	spans []model.RequestTraceSpan,
	stages []requesttrace.Stage,
	statuses []requesttrace.Status,
) {
	t.Helper()
...
func TestRequestTraceCaptureStopsBeforeUpstreamOnGatewayRejections(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			body:       `{"model":"trace-image"}`,
			wantStatus: http.StatusUnauthorized,
			wantStages: []requesttrace.Stage{
				requesttrace.StageRequest,
				requesttrace.StageAuthentication,
			},
			wantStatuses: []requesttrace.Status{requesttrace.StatusError, requesttrace.StatusError},
		},
		{
			name:       "parameter rejection",
			authorized: true,
			body:       `{}`,
		...
		},
		{
			...
			authorized: true,
			body:       `{"model":"trace-image","prompt":"safe"}`,
			...
			prepare: func(t *testing.T, _ *traceCaptureFixture) {
				...
				require.NoError(
					...
					model.DB.Model(&model.Channel{}).
						Where("type = ?", model.ChannelTypeOpenAI).
						Update("status", model.ChannelStatusDisabled).
						Error,
				)
...
func TestRequestTraceCapturePreservesTwoRealRelayAttempts(t *testing.T) {
	...
	require.NoError(
		t,
		model.DB.Exec(
			"UPDATE model_configs SET retry_times = ? WHERE model = ?",
			1,
			"trace-image",
		).Error,
	)
	...
	request := httptest.NewRequestWithContext(context.Background(),
		http.MethodPost,
		"/v1/images/generations",
		strings.NewReader(`{"model":"trace-image","prompt":"retry privately"}`),
	)
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("Authorization", "Bearer trace-test-key")
	request.Header.Set(middleware.RequestIDHeader, requestID)
	...
	for _, span := range spans.Items {
		if span.Stage == requesttrace.StageUpstreamAttempt &&
			span.Status != requesttrace.StatusRunning {
...
func TestRequestTraceEnabledAndDisabledPreserveGatewayAndConsumption(t *testing.T) {
	...
	run := func(t *testing.T, enabled bool) outcome {
		...
		request := httptest.NewRequestWithContext(context.Background(),
			http.MethodPost,
			"/v1/images/generations",
			strings.NewReader(`{"model":"trace-image","prompt":"same"}`),
		)
		request.Header.Set("Content-Type", "application/json")
		request.Header.Set("Authorization", "Bearer trace-test-key")
		request.Header.Set(middleware.RequestIDHeader, requestID)
		...
		require.NoError(
			t,
			model.LogDB.Model(&model.Log{}).Where("request_id = ?", requestID).Count(&count).Error,
		)
		fixture.flush(t)

		return outcome{
			code:     response.Code,
			calls:    int(fixture.providerCalls.Load()),
...
func TestRequestTraceBlockedAndFullQueueDoNotChangeGatewayResult(t *testing.T) {
	...
	baselineRequest := httptest.NewRequestWithContext(context.Background(),
		http.MethodPost,
		"/v1/images/generations",
		strings.NewReader(`{"model":"trace-image","prompt":"unaffected"}`),
	)
	...
	require.NoError(
		t,
		model.LogDB.Model(&model.Log{}).
			Where("request_id = ?", baselineRequestID).
			Count(&baselineConsumes).
			Error,
	)
	...
	require.NoError(
		t,
		model.LogDB.Callback().
			Create().
			Before("gorm:create").
			Register("test:block_trace_writes", func(tx *gorm.DB) {
...
func TestRequestTraceRejectsClientOwnershipAndLeaksNoSensitiveTraceData(t *testing.T) {
	...
	for _, key := range []string{"trace-test-key", "trace-other-key"} {
		request := httptest.NewRequestWithContext(context.Background(),
			http.MethodPost,
			"/v1/images/generations",
			strings.NewReader(`{"model":"trace-image","prompt":"PROMPT_SENTINEL"}`),
		)
		request.Header.Set("Content-Type", "application/json")
	...
	for _, groupID := range []string{"group-a", "group-b"} {
		page, err := fixture.store.FindRequests(
			t.Context(),
			model.TraceRequestQuery{GroupID: groupID, RequestID: requestID},
		)
		require.NoError(t, err)
		require.Len(t, page.Items, 1)
	...
	raw, err := json.Marshal(struct {
		Heads []model.RequestTraceHead `json:"heads"`
		Spans []model.RequestTraceSpan `json:"spans"`
	}{heads, spans})
	require.NoError(t, err)
	...
	apiRequest := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		"/api/trace/group-a/"+traceIDs[0]+"?service=aiproxy",
		nil,
	)
...
func newTraceCaptureFixture(t *testing.T) *traceCaptureFixture {
	...
	provider := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if f.failFirst.Load() && call == 1 {
			...
			_, _ = w.Write(
				[]byte(
					`{"error":{"message":"private upstream failure","type":"server_error","code":"server_error"}}`,
				),
			)
		...
		if r.URL.Path == "/videos" {
			_, _ = w.Write(
				[]byte(
					`{"id":"video_trace","object":"video","status":"queued","model":"trace-video","created_at":1}`,
				),
			)
		...
		_, _ = w.Write(
			[]byte(`{"created":1,"data":[{"url":"https://provider.invalid/private.png"}]}`),
		)
	}))
	...
	require.NoError(t, db.Create(&[]model.Channel{
		{
			...
			Models:  []string{"trace-image"},
		},
		{
			Name:    "trace-image-retry",
			Status:  model.ChannelStatusEnabled,
			Type:    model.ChannelTypeOpenAI,
			Key:     "provider-secret",
			BaseURL: provider.URL,
		...
		},
		{
			...
			Key:     "provider-secret",
			BaseURL: provider.URL,
			Models:  []string{"trace-video::text-to-video"},
		},
	}).Error)
	require.NoError(t, model.InitModelConfigAndChannelCache())
	require.NoError(
		t,
		model.CacheSetGroup(&model.GroupCache{ID: "group-a", Status: model.GroupStatusInternal}),
	)
	require.NoError(
		t,
		model.CacheSetGroup(&model.GroupCache{ID: "group-b", Status: model.GroupStatusInternal}),
	)
	require.NoError(
		t,
		model.CacheSetToken(

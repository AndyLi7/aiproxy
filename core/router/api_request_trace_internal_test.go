...
func TestRequestTraceByRequestRouteReturnsScopedCandidatesAndValidatesAdminHeader(t *testing.T) {
	...
	require.NoError(t, model.LogDB.Create(&[]model.RequestTraceSpan{
		{
			SpanID:    requestTraceID(11),
			TraceID:   requestTraceID(21),
			GroupID:   "group-one",
			RequestID: "duplicate",
		...
		{
			SpanID:    requestTraceID(12),
			TraceID:   requestTraceID(22),
			GroupID:   "group-one",
			RequestID: "duplicate",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageRequest,
		...
		{
			SpanID:    requestTraceID(13),
			TraceID:   requestTraceID(23),
			GroupID:   "group-two",
			RequestID: "duplicate",
			Service:   requesttrace.ServiceAIProxy,
			Stage:     requesttrace.StageRequest,
	...
	req := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		"/api/trace/group-one/by-request/duplicate?limit=1",
		nil,
	)
	...
	req = httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		"/api/trace/group-one/by-request/duplicate?after="+payload.Data.NextCursor+"&limit=1",
		nil,
	)
	req.Header.Set("Authorization", "Bearer admin-key")
...
func TestRequestTraceByRequestRouteReturnsFixedValidationError(t *testing.T) {
	...
	request := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		"/api/trace/group-one/by-request/duplicate?limit=101",
		nil,
	)
	...
	require.JSONEq(
		t,
		`{"success":false,"message":"invalid trace request query"}`,
...
func TestRequestTraceByRequestRouteReturnsFixedUnavailableError(t *testing.T) {
	...
	require.NoError(
		...
		model.LogDB.Callback().
			Query().
			Before("gorm:query").
			Register("test:raw_trace_lookup_failure", func(tx *gorm.DB) {
				require.Error(t, tx.AddError(errors.New("raw-database-sentinel")))
			}),
	)
	...
	request := httptest.NewRequestWithContext(
		context.Background(),
		http.MethodGet,
		"/api/trace/group-one/by-request/duplicate",
		nil,
	)
	request.Header.Set("Authorization", "Bearer admin-key")
...
func TestRequestTraceRouteReportsPersistedTruncationWithoutPaginationCursor(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		"/api/trace/group-one/"+traceID+"?service=aiproxy",
		nil,
	)
	req.Header.Set("Authorization", "Bearer admin-key")
...
func TestRequestTraceRouteReturnsUnavailableWithoutDatabaseDetails(t *testing.T) {
	...
	request := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		fmt.Sprintf("/api/trace/group-one/%s?service=aiproxy", requestTraceID(1)),
		nil,
	)
	request.Header.Set("Authorization", "Bearer admin-key")
...
func TestRequestTraceRouteReadsWriterRetriedLifecycleRefresh(t *testing.T) {
	...
	request := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet,
		fmt.Sprintf("/api/trace/group-one/%s?service=aiproxy", traceID),
		nil,
	)
	request.Header.Set("Authorization", "Bearer admin-key")
...
func requestTraceRouter(t *testing.T, writeLifecycle bool) (*gin.Engine, string) {
	...
	if writeLifecycle {
		...
		writer := requesttrace.NewWriter(
			sink,
			requesttrace.WriterOptions{MaxAttempts: 2, WriteTimeout: time.Second},
		)
		...
		require.Equal(
			...
			int32(3),
			sink.calls.Load(),
			"the initial write retries once and the completion refreshes the same span",
		)

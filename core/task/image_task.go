...
func processOneImageUsage(ctx context.Context, info *model.AsyncUsageInfo) {
	...
	if task.Status == "completed" {
		completePolledAsyncUsage(ctx, info, model.Usage{ImageOutputTokens: model.ZeroNullInt64(len(task.Data))}, info.UsageContext)
		return
	}
	...
	if task.KeyFingerprint != "" && task.KeyFingerprint != model.ImageChannelKeyFingerprint(ch.Key) {
	...
	if result.Status == "completed" && task.ExpectedImages > 0 && len(result.Data) > task.ExpectedImages {
		result = adaptor.ImageTaskResult{Status: "failed", Error: &model.ImageTaskError{Code: "invalid_result", Message: "Unexpected image count"}}
	}
	if err = model.SetImageTaskResult(task.ID, result.Status, result.Data, result.Error); err != nil {
		retryImageUsage(info, err)
		return
	}
	switch result.Status {
	case "completed":
		completePolledAsyncUsage(ctx, info, model.Usage{ImageOutputTokens: model.ZeroNullInt64(len(result.Data))}, info.UsageContext)
	case "failed":
		markAsyncUsageFailed(info, "image generation failed")
	default:

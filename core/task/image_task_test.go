...
func TestImageWorkerPersistsResultBeforeSettlement(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/fal-ai/minimax/requests/upstream/status" {
			...
			w.Write([]byte(`{"images":[{"url":"https://cdn.example/a.png"}]}`))
		}
	}))
	defer s.Close()
	ch := &model.Channel{Type: model.ChannelTypeFal, Key: "secret"}
	require.NoError(t, db.Create(ch).Error)
	info := &model.AsyncUsageInfo{RequestID: "durable", ChannelID: ch.ID, BaseURL: s.URL, GroupID: "g", TokenID: 1}
	_, _, err = model.ReserveImageTask(&model.ImageTask{ID: "durable", GroupID: "g", TokenID: 1, UpstreamModel: "fal-ai/minimax/image-01"}, info)
	require.NoError(t, err)
...
func (c *imageLogReplayConsumer) PostGroupConsume(ctx context.Context, token string, amount float64) (float64, error) {
	var entry model.Log
	require.NoError(c.t, model.LogDB.Where("request_id = ?", balance.RequestIDFromContext(ctx)).First(&entry).Error)
...
func (b imageLogReplayBalance) GetGroupRemainBalance(context.Context, model.GroupCache) (float64, balance.PostGroupConsumer, error) {
	return 100, b.consumer, nil
}
func (imageLogReplayBalance) GetGroupQuota(context.Context, model.GroupCache) (*balance.GroupQuota, error) {
...
func TestImageWorkerPaidSettlementReplayUsesFinalLog(t *testing.T) {
	...
	consumer := &imageLogReplayConsumer{replaySafeAmbiguousAsyncUsageConsumer: replaySafeAmbiguousAsyncUsageConsumer{charges: map[string]int{}}, t: t}
	balance.Default = imageLogReplayBalance{consumer}
	require.NoError(t, model.CacheSetGroup(&model.GroupCache{ID: "image-paid", Status: model.GroupStatusEnabled}))
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(r.URL.Path, "/status") {
			...
			w.Write([]byte(`{"images":[{"url":"https://cdn.example/paid.png"}]}`))
		}
	}))
	defer s.Close()
	ch := &model.Channel{Type: model.ChannelTypeFal, Key: "key"}
	require.NoError(t, db.Create(ch).Error)
	info := &model.AsyncUsageInfo{RequestID: "paid-image", GroupID: "image-paid", TokenID: 1, ChannelID: ch.ID, BaseURL: s.URL, PricingCurrency: "USD", PricingVersion: "release-1", Price: model.Price{ImageOutputPrice: 0.25, ImageOutputPriceUnit: 1}}
	_, _, err = model.ReserveImageTask(&model.ImageTask{ID: "paid-image", GroupID: "image-paid", TokenID: 1, UpstreamModel: "fal-ai/minimax/image-01", ExpectedImages: 1, ChannelType: model.ChannelTypeFal, KeyFingerprint: model.ImageChannelKeyFingerprint(ch.Key)}, info)
	require.NoError(t, err)
	...
	for i := 0; i < 2; i++ {
		...
		require.NoError(t, db.Model(&fresh).Update("next_poll_at", time.Now().Add(-time.Second)).Error)
